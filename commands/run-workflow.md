...
- This installs the file into the session's `.atomic/workflows/`, injects `/workflow reload`, then `/workflow <name>`. If the output mentions an overwrite (`workflow_installed` with `overwrote: true`), tell the user an existing workflow file was replaced.
